...
```
{{PROJECT_ROOT}}/
├── {{BACKEND_DIR}}/                 # {{BACKEND_STACK}} (e.g., Python FastAPI, Go, Node)
│   ├── {{CORE_DIR}}/                # Deterministic core logic (no LLMs, no IO)
│   │   ├── {{DOMAIN_1}}/
│   │   ├── {{DOMAIN_2}}/
│   │   └── {{DOMAIN_N}}/
│   ├── {{SERVICES_DIR}}/            # External integrations, APIs, caches
│   ├── {{API_DIR}}/                 # HTTP/WS handlers, controllers
│   └── {{TESTS_DIR}}/               # Backend tests
├── {{FRONTEND_DIR}}/                # {{FRONTEND_STACK}} (e.g., Next.js, React Native, Swift)
│   ├── src/
│   └── tests/
...
**Deterministic Core Rule:** All business-critical calculations, state machines, and rule engines MUST live inside `{{CORE_DIR}}/`. Never use LLMs or dynamic scripts for deterministic logic. Keep inputs and outputs fully typed.
...
- **Environment path:** `{{VENV_PATH}}` (e.g., `.venv/`, `node_modules/`, `.direnv/`)
...
| Action | Command |
|---|---|
| **Backend dev server** | `{{BACKEND_DEV_CMD}}` |
| **Backend tests** | `{{BACKEND_TEST_CMD}}` |
| **Backend linter** | `{{BACKEND_LINT_CMD}}` |
| **Backend formatter** | `{{BACKEND_FMT_CMD}}` |
| **Frontend dev server** | `{{FRONTEND_DEV_CMD}}` |
| **Frontend build** | `{{FRONTEND_BUILD_CMD}}` |
| **Frontend linter** | `{{FRONTEND_LINT_CMD}}` |
| **Global format/lint** (Main Agent only) | `{{GLOBAL_FMT_CMD}} && {{GLOBAL_LINT_CMD}}` |
...
| Area | Rule |
|---|---|
| **Type Safety** | {{TYPE_SAFETY_RULE}} (e.g., "Strict TypeScript", "Pydantic for all Python models", "Zod for all API boundaries") |
| **Naming** | {{NAMING_CONVENTION}} (e.g., "snake_case for Python, camelCase for TypeScript") |
| **Comments** | Code comments and symbol names must be in **English**. |
| **User-facing text** | Must be in **{{LOCALE_LIST}}** (e.g., "RU and KZ", "EN only", "i18n via `{{I18N_LIB}}`") |
| **Styling** | {{STYLING_RULE}} (e.g., "Tailwind CSS v4", "CSS Modules", "Styled Components") |
...
2. `feat({{MODULE}}): implement feature-X per flow`
3. `test({{MODULE}}): add targeted tests for feature-X flow`
