...
export const BoardPopup: React.FC = () => {
  ...
  return (
    ...
    >
      <div
        ref={contentRef}
        className="relative bg-[#d4c9a8] text-[#1c160e] border-4 border-[#1c160e] shadow-[0_15px_30px_rgba(0,0,0,0.5)] rounded-sm overflow-hidden"
        style={{
          backgroundImage: `
      ...
      >
        ...
        {/* Scrollable inner wrapper for all content (header + body + action button) */}
        <div
          ref={textContainerRef}
          tabIndex={0}
          aria-label="Содержимое дела"
          className="overflow-y-auto max-h-[85vh] sm:max-h-[80vh] p-5 sm:p-8 md:p-10 pr-3 sm:pr-6 md:pr-8 red-dossier-scrollbar focus:outline-none compact-landscape-popup"
        >
          {/* Vintage header */}
          <div className="border-b-2 border-[#1c160e] pb-4 mb-6 compact-landscape-header">
            <h2 id="dialog-title" className="font-display text-2xl md:text-3xl font-extrabold tracking-tight text-[#1c160e] leading-tight select-text compact-landscape-title">
              {data.title}
            </h2>
            {data.subtitle && (
          ...
          <div className={`flex flex-col ${isSuspect ? "sm:flex-row" : ""} gap-6 sm:gap-8 items-center sm:items-start`}>
            {isSuspect && (
              <div className="w-40 sm:w-44 md:w-48 shrink-0 rotate-[-1.5deg] hover:rotate-[1deg] transition-transform duration-300 bg-[#f5f4ef] p-[2%] aspect-square rounded-[2px] border border-[#dfded7] flex flex-col justify-center items-center shadow-inner">
            ...
            <div
              className="font-typewriter text-sm space-y-4 leading-relaxed text-[#2a2217] flex-1 select-text focus:outline-none focus-visible:ring-1 focus-visible:ring-[#1c160e]/40"
            >
          ...
          {data.actionLabel && data.actionUrl && (
            <div className="mt-8 pt-6 border-t border-[#1c160e]/20 flex justify-end compact-landscape-action">
              <a
                href={data.actionUrl}
